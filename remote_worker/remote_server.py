    ...
    try:
        while True:
            ...
            if after > before:
                tries.pop(aid, None)
                continue
            # Ни одной строки не прибавилось. Назначение осталось открытым — без паузы
            # это горячий цикл на пустом месте.
            if not _schedule_permits_loading(state):
                # Закрытое окно — не застревание, а послушание: машина занята хозяином и
                # работать НЕ ДОЛЖНА. Считать это попытками значит отложить всю очередь
                # за полторы минуты рабочего дня — ровно то, что случилось бы на M5.
                await asyncio.sleep(30)
                continue
...
async def _ensure_model_for_work(state: ServerState, loop) -> bool:
    ...
    if not getattr(state, "idle_unloaded", False):
        # Не наша выгрузка. Различать надо два разных состояния, которые раньше были
        # свалены в одно и отвергались оба.
        #
        # Загрузка СОРВАЛАСЬ — за ней стоит отсрочка (`_owe_another_wake` повышает
        # `wake_failures` и ставит `wake_retry_at`), и лезть поверх неё чанком значит
        # разогнать повторные попытки и спрятать настоящую поломку. Отказ остаётся.
        #
        # Модель ПРОСТО ОТСУТСТВУЕТ — ни выгрузки по простою, ни сорванной загрузки, а
        # запомненный спек на месте: так выглядит агент после перезапуска. Этим не
        # владеет никто: контроллер ждёт отсрочки, которой нет, а хост лежит — и M5
        # дважды простоял так с пакетом 0/3197, исправно опрашивая хост и выглядя живым.
        # Работа на руках и открытое окно — достаточная причина вернуть веса.
        if getattr(state, "wake_failures", 0) > 0:
            return False
        if getattr(state, "model_spec", None) is None:
            return False        # возвращать нечего — об этом уже сказал контроллер
        log.info("Pull worker: work arrived with no model and no pending retry — "
                 "restoring the remembered model")
    state.idle_unloaded = False
