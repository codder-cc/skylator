...
if [[ "$OS" == "Darwin" ]]; then
  ...
  if [[ "$ARCH" == "arm64" ]]; then
    ...
    CMAKE_ARGS="-DGGML_METAL=on" pip install llama-cpp-python \
      --no-binary llama-cpp-python --quiet && BACKEND_INSTALLED=true || true
    if ! $BACKEND_INSTALLED; then
      echo "  Metal build failed — trying mlx-lm fallback..."
      pip install mlx-lm --quiet && BACKEND_INSTALLED=true || true
    fi
  else
    echo "  Installing llama-cpp-python (Intel Mac)..."
    pip install llama-cpp-python --quiet && BACKEND_INSTALLED=true || true
  fi
elif [[ "$OS" == "Linux" ]]; then
  echo ""
  ...
  pip install llama-cpp-python \
    ...
    --quiet && BACKEND_INSTALLED=true || true
  if ! $BACKEND_INSTALLED; then
    echo "  Pre-built CUDA wheel failed — installing CPU fallback..."
    pip install llama-cpp-python --quiet && BACKEND_INSTALLED=true || true
  fi
fi

if $BACKEND_INSTALLED; then
  echo "✓  Inference backend installed"
else
  echo "⚠  Backend install failed — you can install manually later"
  echo "   See remote_worker/README.md for instructions"
fi
...
python server.py --host-url "$HOST_URL"
"""
