...
def main() -> None:
    ...
    for r in con.execute(
            "SELECT original, translation, rec_type FROM strings "
        ...
        # Одна и та же пара встречается в разных модах; считать её дважды значит
        # взвесить ответник по популярности мода, а не по строкам.
        if (en, ru) in seen:
            continue
        seen.add((en, ru))
        off = off.strip()
