...
def test_an_address_noun_moves_only_from_a_short_certain_list():
    """«Грубиян» → «грубиянка» — словообразование, а не склонение: морфология не может.

    Промпт тоже не может: пять формулировок, включая нарочито жёсткую и с образцом,
    оставили «грубиян» нетронутым. Остаётся короткий список бесспорных пар — и он
    короткий намеренно: таких мест 54 на 463 тысячи строк, половина из них ложные
    (имена «Шанат», «Дельфин»), а для большинства занятий мужская форма и есть верная.
    """
    assert G.enforce_addressee("", "Ты грубиян.", "f") == "Ты грубиянка."
    assert G.enforce_addressee("", "Ты дурак.", "f") == "Ты дура."


def test_a_profession_keeps_the_masculine_because_russian_does():
    # «Ты художник?» женщине по-русски нормально, «художница» звучала бы хуже.
    assert G.enforce_addressee("", "Ты художник?", "f") == "Ты художник?"


def test_a_noun_with_an_adjective_is_left_alone():
    """«Ты эгоистичный дурак» → «эгоистичный дура» рвёт согласование.

    Правка одного слова из пары хуже нетронутой фразы — тот же довод, что и у
    цепочки однородных сказуемых. Склонять оба значило бы решать две задачи там,
    где таких мест единицы.
    """
    text = "Ты эгоистичный дурак."
    assert G.enforce_addressee("", text, "f") == text
